...
#existe quel est ce chemin. Pour ce, on construit une sous fonction renvoyant le chemin, s'il existe,
    ...
    def get_path_with_power2(self, src, dest, power):
        verif={node:False for node in self.nodes} 
        verif[src] = True
        queue = [(src, [src])] # Pile de noeuds à visiter et de chemins parcourus
        while queue!=[]:
            node, path = queue.pop() # Récupération du noeud et du chemin actuel
            if node == dest: # Condition d'arrêt: on a atteint la destination
                return path
            for ngbr in self.graph[node]:
                if not verif[ngbr[0]] and ngbr[1] <= power:
                    verif[ngbr[0]] = True
                    new_path = path + [ngbr[0]] # Création d'un nouveau chemin en ajoutant le voisin
                    queue.append((ngbr[0], new_path))
        return None
...
#puissance minimale recherchée.
    ...
    def min_power(self, src, dest):
        ...
        #while self.get_path_with_power2(src, dest, d)==None: #condition nécessaire si au début une 
